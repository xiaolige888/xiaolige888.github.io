...
if (navigator.userAgent.match(/(iPhone|iPod|iPad|Android|ios|Nokia|Black Berry|MIDP|Phone)/i) && mobiles === "no") {
	...
	if (myhkLoad && window.location.href.indexOf("music.piphp.com") < 0) {
		$("#myhkplayer").hide()
	} else {
		var songSheetList;
		var webURL = 'https://music.piphp.com/';
		var keyId = $("#xplayer").attr("key");
		console.log("依米花音乐播放器插件：" + webURL);
...
"<div id=\"myhkLrc\"></div>");
		if (top.location != self.location) {
			...
			$songFrom.html("<a style=\"color:#f00\">依米花</a>");
				...
				creat: {
					album: function() {
						if (typeof myhkid === "undefined") {
							...
							$(".musicheader", $albumList).html("<a href=\"" + webURL + "\" title=\"欢迎使用依米花播放器</a>")
						};
			...
			$.ajax({
				...
				success: function() {
					if (typeof myhkid !== "undefined") {
						$(".myhkgeci", $player).html("<a style=\"float: right\" href=\"" + webURL + "\" title=\"依米花播放器\" target=\"_blank\"><i class=\"fa fa-share-alt\"></i> 免费申请</a>");
